...
void Skelly::updateMovements() {
  ...
  std::vector<float> values = {0, 8, 45, 5, -15, 5, 0, 0};
...
void Skelly::updateFeet(Time now) {
  updateHeel(now, rig_.lfoot_m_, walk_.lheel);
  updateHeel(now, rig_.rfoot_m_, walk_.rheel);
  updateFoot(rig_.rfoot_m_, *rig_.rfoot_, now, walk_.rstep);
  updateFoot(rig_.lfoot_m_, *rig_.lfoot_, now, walk_.lstep);
}

void Skelly::updateHeel(Time now, FootMeta& foot_m, Movement<float>& move) {
  if (move.should_start) {
    startMovement(move, now);
  }

  foot_m.angle = glm::radians(sampleMovement(move, now));
  if (move.anim && now > move.anim->to_time_) {
    move.anim.reset();
    foot_m.angle = 0;
  }

  // foot_m.angle += sampleMovement(walk_.heels_add, now);
}

void Skelly::updateFoot(
...
void Skelly::updateLegs() {
  updateLeg(
      *rig_.lhip_, *skeleton_.lshin_, *rig_.lfoot_, *rig_.ltoe_, rig_.lfoot_m_);
  updateLeg(
      *rig_.rhip_, *skeleton_.rshin_, *rig_.rfoot_, *rig_.rtoe_, rig_.rfoot_m_);
}

// TODO: Use updateTwoBoneIk() for most of this.
void Skelly::updateLeg(
    Object& hip, Object& shin, Object& foot, Object& toe, FootMeta& foot_m) {
  float toe_angle = 0;
  // If hip is too far from ankle, compute the angle to lift the heel just
  // enough to compensate.
  vec3 hip_pos = hip.posToAncestor(&root_);
  // vec3 ankle_pos = toe.toAncestor(&root_) * vec4(sizes_.ankle, 1);
  vec3 ankle_pos = foot.getPos();
  // foot.setRot(glm::identity<glm::quat>());
  vec3 toe_pos = toe.posToAncestor(&root_);
  // float hip_to_ankle = glm::length(hip_pos - ankle_pos);
  // float max_leg = 0.97 * (sizes_.ankle_d);
  // if (hip_to_ankle > max_leg) {
  //   float hip_to_toe = glm::length(toe_pos - hip_pos);
  ...
  //   }
  // }
  float toe_target = std::min(toe_pos.y, 5.f);
  // foot_m.angle = toe_angle;

  glm::quat ankle_rot = glm::angleAxis(toe_angle, vec3(1, 0, 0));
  vec3 ankle_in_toe = toe.posToLocal(&foot);
  vec3 raise_ankle = ankle_rot * ankle_in_toe;
  // foot.setPos(toe.posToAncestor(&root_, raise_ankle));

  // if (foot_m.is_left) {
  //   std::println(
  //       "ang:{} ankleintoe:{}, raisepos:{}", glm::degrees(toe_angle),
  //       toStr(ankle_in_toe), toStr(raise_ankle));
  // }
  ...
  //   vec3 target = root_.toLocal() * vec4(foot_m.world_target, 1);
  //   float above = toe_pos.y - target.y;
  //   float lift =
  ...
  // vec3 new_toe_pos = toe.posToAncestor(&root_);
  // if (new_toe_pos.y < 0) {
  //   float lift = -new_toe_pos.y;
  //   vec3 ankle_in_toe
  //   vec3 new_pos = ankle_rot * foot.getPos();
  //   foot.setPos(new_pos);
  // }

  mat4 foot_to_root = shin.toLocal(&root_);
  glm::quat foot_rot = ankle_rot * glm::quat_cast(foot_to_root);
  foot.setRot(glm::angleAxis(foot_m.angle, vec3(1, 0, 0)));
}
